...
Song::Song()
    : reference_(0), artist_(-1), albumArtist_(-1), album_(-1), track_(-1),
      genre_(-1), date_(-1), disc_(-1), duration_(0), virtualEnd_(0), uri_(""),
      title_(""), lastFormat_(""), formatted_(""), entry_(NULL) {}

Song::Song(Song const &song)
    : reference_(0), artist_(song.artist_), albumArtist_(song.albumArtist_),
      album_(song.album_), track_(song.track_), genre_(song.genre_),
      date_(song.date_), disc_(song.disc_), duration_(song.duration_),
      uri_(song.URI()), title_(song.Title()), lastFormat_(song.lastFormat_),
...
int32_t Song::Reference() const { return reference_; }
...
void Song::AddSlashes(std::string &String) {
  ...
  for (std::string::size_type i = 0;
       (i = String.find(find, i)) != std::string::npos;) {
    String.replace(i, find.length(), replace);
    i += replace.length();
  }
}

void Song::Set(const char *newVal, int32_t &oldVal,
...
std::string const &Song::AlbumArtist() const {
  if ((albumArtist_ >= 0) &&
      (albumArtist_ < static_cast<int32_t>(Artists.size()))) {
...
void Song::SetAlbum(const char *album) { Set(album, album_, Albums, AlbumMap); }
...
std::string const &Song::Title() const { return title_; }

void Song::SetTrack(const char *track) { Set(track, track_, Tracks, TrackMap); }
...
std::string const &Song::URI() const { return uri_; }

void Song::SetGenre(const char *genre) { Set(genre, genre_, Genres, GenreMap); }
...
void Song::SetDate(const char *date) { Set(date, date_, Dates, DateMap); }
...
void Song::SetDisc(const char *disc) { Set(disc, disc_, Discs, DiscMap); }
...
int32_t Song::Duration() const { return duration_; }

void Song::SetVirtualEnd(int32_t end) { virtualEnd_ = end; }

int32_t Song::VirtualEnd() const { return virtualEnd_; }

void Song::SetEntry(LibraryEntry *entry) { entry_ = entry; }

LibraryEntry *Song::Entry() const { return entry_; }
...
std::string Song::ParseString(std::string::const_iterator &it,
                              bool valid) const {
  std::string result;
